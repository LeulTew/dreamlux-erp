...
async function runEventsMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, "migrations", "event_lifecycle.sql");
    const migrationSql = fs.readFileSync(migrationPath, "utf-8");

    console.log("Running events schema migrations...");
    await client.query(migrationSql);
    console.log("Events schema migrations completed successfully!");
