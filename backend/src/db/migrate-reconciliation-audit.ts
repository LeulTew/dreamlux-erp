...
async function runReconciliationAuditMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, "migrations", "reconciliation_audit.sql");
    const migrationSql = fs.readFileSync(migrationPath, "utf-8");

    console.log("Running reconciliation audit migrations...");
    await client.query(migrationSql);
    console.log("Reconciliation audit migrations completed successfully!");
  } catch (error) {
...
runReconciliationAuditMigration();
