...
async function runReconciliationAuditHardeningMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, "migrations", "reconciliation_audit_hardening.sql");
    const migrationSql = fs.readFileSync(migrationPath, "utf-8");

    console.log("Running reconciliation audit hardening migrations...");
    await client.query(migrationSql);
    console.log("Reconciliation audit hardening migrations completed successfully!");
  } catch (error) {
...
runReconciliationAuditHardeningMigration();
