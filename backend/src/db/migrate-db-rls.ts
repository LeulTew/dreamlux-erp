...
async function runDbRlsHardeningMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, "migrations", "db_rls_hardening.sql");
    const migrationSql = fs.readFileSync(migrationPath, "utf-8");

    console.log("Running DB RLS hardening migration...");
    await client.query(migrationSql);
    console.log("DB RLS hardening migration completed successfully!");
  } catch (error) {
