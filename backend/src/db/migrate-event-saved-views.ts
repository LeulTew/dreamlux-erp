...
async function runEventSavedViewsMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, "migrations", "event_saved_views.sql");
    const migrationSql = fs.readFileSync(migrationPath, "utf-8");

    console.log("Running event saved views migration...");
    await client.query(migrationSql);
    console.log("Event saved views migration completed successfully!");
  } catch (error) {
