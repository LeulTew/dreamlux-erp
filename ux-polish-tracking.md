...
| E | [#81 Notification center, routing matrix, and permission-safe delivery](https://github.com/LeulTew/dreamlux-erp/issues/81) | Correction: PR #92 does not exist, so the prior tracker completion claim was invalid. Notification UI exists on `main`, but backend delivery logic needed hardening for multi-role permission fanout, actor/system labeling, category preference matching, and CRUD/workflow audit readiness. | `feature/81-notification-routing-hardening` hardens backend routing logic only; create the real PR after verification. | `project-context.md`, `RULES.md`, AGENTS instructions, `docs/SENIOR_ISSUE_REVIEW_PROMPT.md`; Supabase/security checklist; Bun backend/frontend verification before PR. | In progress. |
