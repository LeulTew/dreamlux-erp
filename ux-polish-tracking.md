...
| #79 Unified trash and restore | Implemented locally | Branch `feature/79-unified-trash-restore` adds event/proposal trash APIs, permission-aware trash pages, pagination, audit logs, and conservative hard-delete blocking. PR/CI pending; do not merge until explicitly authorized. |
...
| #79 | Unified trash and restore for events, proposals, and setup records | Branch `feature/79-unified-trash-restore`; issue #79 open; PR pending. Local implementation covers event/proposal trash listing, restore, permanent delete, audit logging, permission-aware UI, and bounded pagination. | Implemented locally. #77 setup doctypes were audited and left unchanged because departments/offices/positions do not currently have `deleted_at`; no destructive migration was forced. |
...
| C | [#79 Unified trash and restore for events, proposals, and setup records](https://github.com/LeulTew/dreamlux-erp/issues/79) | Local branch implements event/proposal trash list, restore, soft delete, permanent delete, audit logs, UI links/pages, and bounded pagination. Permanent event delete blocks operational dependencies; converted proposals cannot be hard-deleted. | Finish browser smoke, commit, push, open PR, and verify remote CI. Do not merge unless user explicitly authorizes. | Local checks passed: backend events test 109/109, backend full test 317/317, backend lint/build, frontend trash test 4/4, frontend full test 99/99, frontend lint/build, and `git diff --check`. | Implemented locally / PR pending. |
