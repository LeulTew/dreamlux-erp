...
| F | [#82 Record activity timeline and audit drawer](https://github.com/LeulTew/dreamlux-erp/issues/82) | Existing implementation already added `activity_logs`, `/api/activity`, `ActivityService`, event/proposal normalization, and shared `ActivityDrawer` on event/proposal detail pages. Final hardening in progress adds entity permission checks, bounded feed queries, source metadata, encoded drawer requests, and drawer/API tests. | Complete final security/QA pass, then PR/merge once backend/frontend tests, lint, builds, senior diff review, and GitHub Actions are clean. | Senior review prompt for audit/PII; frontend skill; backend BOLA/BFLA checks; tests for ordering/redaction/actor fallback/source metadata/drawer behavior. | In progress on `feature/82-activity-timeline-drawer`; targeted activity API and drawer tests pass. |
