...
| #84 PWA Installability | Closed | [PR #102](https://github.com/LeulTew/dreamlux-erp/pull/102) completed; SW, manifest, offline route, and pre-prompt notifications tests. |
| #85 HR Dashboard | Closed | [PR #100](https://github.com/LeulTew/dreamlux-erp/pull/100) / [PR #103](https://github.com/LeulTew/dreamlux-erp/pull/103) completed; HR metrics, exception center, and E2E tests. |
| #86 Custom role manager UX | Closed | [PR #104](https://github.com/LeulTew/dreamlux-erp/pull/104) completed; permissions search, diff preview, dangerous validation modals, deletion blocks, and E2E tests. |
...
| #84 | PWA installability, offline shell, and notification readiness | [PR #102](https://github.com/LeulTew/dreamlux-erp/pull/102) completed; includes `sw.js`, `manifest.ts`, `/offline` page, and pre-prompt desktop alerts. | Complete. 5/5 Vitest unit tests and 6/6 Playwright E2E tests pass. All pipeline checks are green. |
| #85 | HR dashboard for workforce, payroll, and staffing readiness | [PR #100](https://github.com/LeulTew/dreamlux-erp/pull/100) / [PR #103](https://github.com/LeulTew/dreamlux-erp/pull/103) completed; HR metrics, exception center, and E2E tests. | Complete. Gated behind hr:read / payroll:read; all unit and Playwright tests pass. |
| #86 | Custom role manager UX guardrails and auditability | [PR #104](https://github.com/LeulTew/dreamlux-erp/pull/104) completed; search, diff preview, dangerous validation modals, deletion blocks, and E2E tests. | Complete. Search, diff preview, and confirmation modal are fully operational; 8/8 E2E tests pass. |
...
| D | [#80 Seeded admin and user credentials parity](https://github.com/LeulTew/dreamlux-erp/issues/80) | PR #91 merged to `main`; issue #80 is closed. Canonical dev/test users are now `admin`, `ceo`, `ops`, `acc`, `eventmgr`, `inv`, `inventory_user`, and `driver`, all documented with `Password123` for local QA only. Driver maps to Selam Bekele via `selam@dreamlux.com`; admin bootstrap refreshes stale dev/test admin hashes without committed live hashes or secrets. | No remaining #80 implementation work. `docs/TEST_CREDENTIALS.md` and `docs/test-plan.md` were updated for the credential parity matrix and non-technical QA flow. | Final verification before merge: backend `bun test` = 322 pass, backend lint/build pass; frontend `bun run test` = 99 pass, frontend lint/build pass; Playwright `phase5-access-polish.spec.ts` = 20 pass and `issue79-trash.spec.ts` = 4 pass; GitHub Actions `backend-test` and `frontend-build` passed. | [Completed (PR #91)](https://github.com/LeulTew/dreamlux-erp/pull/91). |
| E | [#81 Notification center, routing matrix, and permission-safe delivery](https://github.com/LeulTew/dreamlux-erp/issues/81) | Integrated live Realtime web-sockets, unread count badge, custom toast notification listener, query cache invalidations, and fixed backend race conditions. | PR #98 handles multi-role delivery. Pushed commits `edd40cd`, `5890aa6`, `50bb4c1`, and `3c6e050` handle realtime web-sockets, global MutationCache, backend await race condition fixes, unread count tracking PremiumToasts, and unit test suite. | Verified: 104 passed frontend tests, 342 passed backend tests, RLS select policy enabled on public.notifications, backend-test & frontend-build CI success. | [Completed (PR #98 & Main Commits)](https://github.com/LeulTew/dreamlux-erp/pull/98). |
| F | [#82 Record activity timeline and audit drawer](https://github.com/LeulTew/dreamlux-erp/issues/82) | Existing implementation added `activity_logs`, `/api/activity`, `ActivityService`, event/proposal normalization, and shared `ActivityDrawer` on event/proposal detail pages. Final hardening added entity permission checks, bounded feed queries, source metadata, encoded drawer requests, notification lint cleanup, and focused drawer/API coverage. | No remaining implementation work after final security/QA pass and merge. Shared activity feed is permission-gated, redacted, and verified on proposal detail desktop/mobile flows. | Senior review prompt for audit/PII; backend BOLA/BFLA checks; frontend drawer tests; targeted Playwright desktop/mobile drawer coverage; backend/frontend lint and builds. | [Completed (PR #99)](https://github.com/LeulTew/dreamlux-erp/pull/99). |
| G | [#83 Security posture status page with OWASP/CVE tracking](https://github.com/LeulTew/dreamlux-erp/issues/83) | Added `/settings/security` as a hidden admin-only posture page, linked from Settings with breadcrumb support. It reports security posture, software safety, data protection, activity history, and known caveats without exposing secrets or runtime env values. Fully redesigned into plain English/Amharic with zero technical jargon. | No remaining implementation work. Redesigned with premium `rounded-xl` border-radius. 20/20 Vitest tests cover all interactions, security/credentials scan, and Amharic i18n. Merged with all CI checks passing. | Verified locally and in GitHub Actions CI: workspace `bun run test` (147 passed), `bun run build` successful. | [Completed (PR #101)](https://github.com/LeulTew/dreamlux-erp/pull/101) |
| H | [#84 PWA installability, offline shell, and notification readiness](https://github.com/LeulTew/dreamlux-erp/issues/84) | Added `manifest.webmanifest`, branded icons, a conservative same-origin service worker, `/offline` fallback page, and `PwaLifecycle` install flow. Notification permission prompting was tightened so native permission requests only happen from explicit user action, and authenticated API responses are not cached offline. | No remaining implementation work. Fully integrated and verified. | Verified locally and in GitHub Actions CI: workspace `bun run test` (147 passed), `bun run build`, and Playwright E2E tests `e2e/issue84-pwa-offline.spec.ts` passed. | [Completed (PR #102)](https://github.com/LeulTew/dreamlux-erp/pull/102) |
| I | [#85 HR dashboard for workforce, payroll, and staffing readiness](https://github.com/LeulTew/dreamlux-erp/issues/85) | Dedicated HR/payroll dashboard with KPI cards, document metrics, exception lists, click-to-edit drawer integration, and Amharic translations. | No remaining implementation work. Fully integrated and verified. | Verified locally and in GitHub Actions CI: workspace `bun run test` (147 passed), `bun run build`, and Playwright E2E tests `e2e/issue85-hr-dashboard.spec.ts` passed. | [Completed (PR #100 / PR #103)](https://github.com/LeulTew/dreamlux-erp/pull/100) |
| J | [#86 Custom role manager UX guardrails and auditability](https://github.com/LeulTew/dreamlux-erp/issues/86) | Backend custom role APIs and activity log writing exist; frontend UI exists in settings/permissions/page.tsx. | Enhances UI with search filter, diff previews, warning badges, dangerous confirms, deletion blocks, and touch-safety. | Verified: 15/15 backend RBAC tests pass, frontend lints/builds pass, Playwright E2E tests pass. | [Completed (PR #104)](https://github.com/LeulTew/dreamlux-erp/pull/104) |
| K | [#87 Pagination inventory and unbounded-list hardening](https://github.com/LeulTew/dreamlux-erp/issues/87) | Branch `feature/87-pagination-hardening`. Audit confirmed primary list pagination exists across dashboard/events/proposals/assets/trash/history/low-stock/location/reconcile/salary/profit/payment-detail/expense-approval/event-types. Fixed real gaps: `/payroll/runs` now returns capped server pagination metadata, including bounded SQL aggregate pagination for `sortBy=total`; `/api/activity` now exposes explicit `page`, `limit`, and `hasMore` with parameterized limits. | HR payments consumes server pages; payroll generation and global search consume bounded payroll history payloads; activity drawer requests an explicit bounded page; lint-only blockers in existing tests were cleaned to keep zero-warning validation. Bounded snapshot calls (`getEmployees(1,1000/5000)`) remain intentional administrative snapshots, not unbounded table rendering. | Verified locally: focused payroll/activity/drawer tests pass; workspace `bun run test` = backend 347 pass + frontend 149 pass; workspace `bun run lint` pass; workspace `bun run build` pass; Playwright `issue82-activity-drawer.spec.ts` = 2 pass and payroll read-only smoke = 2 pass. `gh` CLI unavailable locally, so PR/CI must be created/checked via GitHub UI or another GitHub-capable environment. | Implementation complete locally; PR pending. |
