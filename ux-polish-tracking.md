...
| K | [#87 Pagination inventory and unbounded-list hardening](https://github.com/LeulTew/dreamlux-erp/issues/87) | PR #105 merged to `main`; issue #87 is closed. Audit confirmed primary list pagination exists across dashboard/events/proposals/assets/trash/history/low-stock/location/reconcile/salary/profit/payment-detail/expense-approval/event-types. Fixed real gaps: `/payroll/runs` now returns capped server pagination metadata, including bounded SQL aggregate pagination for `sortBy=total`; `/api/activity` now exposes explicit `page`, `limit`, and `hasMore` with parameterized limits. | No remaining implementation work. HR payments consumes server pages; payroll generation and global search consume bounded payroll history payloads; activity drawer requests an explicit bounded page. Bounded snapshot calls (`getEmployees(1,1000/5000)`) remain intentional administrative snapshots, not unbounded table rendering. | Verified locally: focused payroll/auth tests = 57 pass; backend `bun test` = 347 pass; previous workspace checks for PR #105 included backend 347 pass, frontend 149 pass, lint pass, build pass, Playwright `issue82-activity-drawer.spec.ts` = 2 pass and payroll read-only smoke = 2 pass. GitHub Actions on PR #105 passed `backend-test` and `frontend-build` before merge. | [Completed (PR #105)](https://github.com/LeulTew/dreamlux-erp/pull/105). |
