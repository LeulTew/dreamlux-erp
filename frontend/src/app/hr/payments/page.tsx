      ...
      <div className="page-container pt-4 md:py-8 px-4 sm:px-6 md:px-8 space-y-6 md:space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 bg-card/40 backdrop-blur-md p-4 rounded-2xl border border-border/50 shadow-sm">
          <div className="space-y-1.5">
            <label className="text-[9px] font-black uppercase text-muted-foreground tracking-widest pl-1">Year</label>
            <select 
              value={yearFilter}
          ...
          <div className="space-y-1.5">
            <label className="text-[9px] font-black uppercase text-muted-foreground tracking-widest pl-1">Status</label>
            <select 
              value={statusFilter}
          ...
          <div className="space-y-1.5">
            <label className="text-[9px] font-black uppercase text-muted-foreground tracking-widest pl-1">Sort By</label>
            <select 
              value={sortBy}
              onChange={(e) => { setSortBy(e.target.value); setPage(1); }}
              className="w-full h-10 px-4 py-2 bg-card border border-border/80 rounded-xl text-xs font-bold focus:ring-4 focus:ring-primary/10 outline-none transition-all"
            >
              <option value="period_start">Period Date</option>
          ...
          <div className="space-y-1.5">
            <label className="text-[9px] font-black uppercase text-muted-foreground tracking-widest pl-1">Order</label>
            <button
                  ...
                  <div className="flex flex-col gap-6 md:flex-row md:items-center relative z-10">
                    ...
                    <div className="flex flex-wrap items-center gap-8 md:gap-12">
                      <div className="text-left md:text-right">
                        <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-black mb-1 opacity-70">Payout Total</p>
                        <p className="text-lg font-black tracking-tighter text-foreground">ETB {Number(run.total_payroll_value ?? 0).toLocaleString()}</p>
                      </div>
                      <div className="text-left md:text-right">
                        <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-black mb-1 opacity-70">Employees</p>
        ...
        <DeleteConfirmModal 
          ...
          confirmLabel={confirmState?.action === "restore" ? "Confirm Restore" : undefined}
          title={
          ...
          message={
            confirmState?.action === "restore"
              ? "Restore this payroll run to the active list?"
              : confirmState?.action === "delete"
                ? "Permanently delete this payroll run and all related lines? This cannot be undone."
                : "Archive this payroll run? It will be removed from the active list and moved to trash."
          }
