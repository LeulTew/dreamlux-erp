...
function EmployeesPageContent() {
  ...
  useEffect(() => {
    if (data && rawEmployees.length === 0 && total > 0 && page > 1) {
      setPage(Math.min(page - 1, Math.ceil(total / limit)));
    }
  }, [rawEmployees, total, page, limit, data]);
            ...
            <button
              ...
              className={`flex items-center gap-1.5 h-9 2xl:h-11 px-3 2xl:px-4 rounded-2xl text-xs 2xl:text-sm font-semibold transition-all ${
                showTrash
                  ...
                  : "bg-card-alt text-muted border border-border hover:bg-rose-500/10 hover:border-rose-500/30 hover:text-rose-600 dark:hover:bg-rose-500/20 dark:hover:border-rose-500/40 dark:hover:text-rose-400"
              }`}
            >
              <HiTrash className="w-4 h-4" />
